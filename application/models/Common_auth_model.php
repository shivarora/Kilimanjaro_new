...
class Common_auth_model extends CI_Model {
	...
	function login()
	{
		...
		*/
		...
		if ($this->form_validation->run())
		{
			...
			// Verify login data.
			$verified_status = $this->flexi_auth->login($this->input->post('login_identity'), $this->input->post('login_password'), $remember_user);

			// Save any public status or error messages (Whilst suppressing any admin messages) to CI's flash session data.
			$this->session->set_flashdata('message', $this->flexi_auth->get_messages());			
//			var_dump( $verified_status);
//                        die();
	...
	function update_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Get user profile data from input.
			// IMPORTANT NOTE: As we are updating multiple tables 
			// (The main user account and user profile tables), it is very important to pass the
			// primary key column and value in the $profile_data for any custom user tables being updated, 
...
/* End of file common_auth_model.php */
/* Location: ./admin/models/common_auth_model.php */