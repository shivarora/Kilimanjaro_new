...
class Flexi_auth extends Flexi_auth_lite
{
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE) 
	{
		if ($this->CI->flexi_auth_model->login($identity, $password, $remember_user))
		{
			...
			if($this->CI->session->userdata( 'cartAssocDet' )) {
      			$this->CI->session->unset_userdata( 'cartAssocDet' );
			}
			if($this->CI->session->userdata( 'cartReorderRef' )) {
      			$this->CI->session->unset_userdata( 'cartReorderRef' );	
			}
			if($this->CI->session->userdata( 'cart_contents' )) {
	...
	public function activate_user($user_id, $activation_token = FALSE, $verify_token = TRUE)
	{
		...
		{			
			$user_grp_where = [ $this->CI->auth->tbl_col_user_privilege_users['user_id'] => $user_id];
			$user_privileges = $this->get_user_privileges_query(FALSE, $user_grp_where)->result_array();
			
			if( !$user_privileges && !sizeof( $user_privileges )){
				$grp_where = [ $this->CI->auth->tbl_col_user_account['id'] => $user_id];
				$user_grp_det = $this->get_users_group_query(FALSE, $grp_where )->row_array();
				
				$grp_where = [ $this->CI->auth->tbl_col_user_privilege_groups['group_id'] => $user_grp_det[ 'ugrp_id' ]];
				$grp_privileges = $this->get_user_group_privileges_query(FALSE, $grp_where )->result_array();
				
				if( $grp_privileges ){					
					foreach( $grp_privileges as $stackInd => $grpDet ){
						$this->insert_privilege_user($user_id, $grpDet['upriv_id']);
					}					
				}				
			}
			$grp_where = [ $this->CI->auth->tbl_col_user_account['id'] => $user_id];
			$user_grp_det = $this->get_users_group_query( $grp_where );
	...
	/**
	 * change_password
	 * Validates a submitted 'Current' password against the database, 
	 * if valid, the database is updated with the 'New' password. 
	 *
	 * @return bool
	 * @author Mathew Davies
	 */
	...
	/**
	 * forgotten_password
	 * Sends the user an email containing a link the user must click to verify they have requested to change 
	 ...
	 * This function is similar to the above 'validate_forgotten_password()' function, 
	 * however, if validated the function also updates the database
	 ...
	 * Sends the user a verification email to their new email address, the user must then click a link within 
	 * the email to update their accounts email address.
	 * This will help prevent a user accidentally locking themselves out of their account if they forget their password, 
	 * as they can request a new password
	...
	public function insert_user($email, $username = FALSE, $password, $user_data, $group_id = FALSE, $activate = FALSE) 
	{
		...
		if ($user_id)
		{
			...
			if (!empty($activation_token))
			{
				...
				$template = $this->CI->auth->email_settings['email_template_directory'].
							$this->CI->auth->email_settings['email_template_activate'];
	 ...
	 * @author Rob Hussey
	 */
	public function search_users_query($search_query = FALSE, $exact_match = FALSE, $sql_select = FALSE, $sql_where = FALSE, $sql_group_by = TRUE, $escape = TRUE)
	{
		return $this->CI->flexi_auth_model->search_users($search_query, $exact_match, $sql_select, $sql_where, $sql_group_by, $escape);
	}
	 ...
	 * change flex setting key to mentioned value
	 *
	 * @author Vishal Suri
	 */
	public function change_config_setting($key, $val){
		$this->CI->auth->auth_settings[$key] = $val;
	}
}
