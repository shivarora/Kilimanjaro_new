...
class Commonmodel extends CI_Model {
    ...
    public function insert_bulk($param = array(), $table_name = null) {
        $this->reset_db();
        $from_tbl = $this->tbl_name;
		if ( !empty($table_name) ) {
            $from_tbl = $table_name;
        }                
        return $this->db->insert_batch($from_tbl, $param);
    }
    ...
    public function delete_record($param = array(), $table_name = null) {
        $this->reset_db();
        $this->param_action($param);
        $from_tbl = $this->tbl_name;
		if ( !empty($table_name) ) {
        ...
        return $this->db->delete($from_tbl);
    }
        ...
        if (sizeof($param) && is_array($param)) {
            foreach ($param as $actionK => $actionDet) {
                switch ($actionK) {
                    ...
                    case 'order':
                        $ordAct = com_arrIndex($actionDet, 1, 'asc' );
                        $this->db->order_by($actionDet[ 0 ], $ordAct);
                        break;
                }
            }
        }
            ...
            foreach ($where_checks as $checkK => $checkD) {
                switch ($checkK) {
                    case 'in_array':
                        ...
                        break;
                    case 'like': case 'or_like': case 'not_like': case 'or_not_like':
                        foreach ($checkD as $subcheckK => $subcheckVal) {
                            $fld = com_arrIndex($subcheckVal, 0, '');
                            $match = com_arrIndex($subcheckVal, 1, '');
                            $side = com_arrIndex($subcheckVal, 2, 'both');
                            $escp = com_arrIndex($subcheckVal, 3, FALSE);
                            $this->db->$checkK($fld, $match, $side, $escp);
                        }
                        break;
                    case 'and_array':
                        foreach ($checkD as $subcheckK => $subcheckVal) {
                            $this->db->where($subcheckVal[0], $subcheckVal[1]);
                        }
                        break;
                    case 'in':
                        if (isset($checkD['col'])) {
    ...
    private function param_db_join($joins) {
        foreach ($joins as $join_det) {
            if (isset($join_det['alias']) && $join_det['alias']) {
                $join_det['tbl'] = $this->db->dbprefix($join_det['tbl']) . ' as ' . $join_det['alias'];
            }
            if (isset($join_det['pass_prefix']) && $join_det['pass_prefix']) {
                $this->db->join($join_det['tbl'], $join_det['cond'], $join_det['type'], FALSE
                );
            } else {
                $this->db->join($join_det['tbl'], $join_det['cond'], $join_det['type']
                );
            }
        }
    }
