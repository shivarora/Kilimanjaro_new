...
class Flexi_auth_lite_model extends CI_Model
{
	 ...
	 * @author Rob Hussey
	 */
	public function get_users($sql_select = FALSE, $sql_where = FALSE, $sql_group_by = TRUE, $escape = TRUE)
	{
		...
		$this->set_custom_sql_to_db($sql_select, $sql_where, $escape);
	 ...
	 * Used internally by flexi auth to call any custom user defined SQL Active Record functions at 
	 * the correct point during a function.
	 *
	 * @return null
	 * @author Rob Hussey
	 */
	public function set_custom_sql_to_db($sql_select = FALSE, $sql_where = FALSE, $escape = TRUE ) 
	{		
		...
		foreach($clause_types as $sql_clause)
		{
			...
			if (! empty($this->auth->$sql_clause))
			{
				...
				foreach($this->auth->$sql_clause as $value)
				{
					...
					{
						if( $sql_clause == 'select' &&  !$escape) {
							$this->db->$sql_clause($value, $escape);
						} else {
