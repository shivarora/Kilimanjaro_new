...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	public function insert_user($email, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
		...
		$activation_token = $this->auth->auth_settings['shoot_email_on_account_create'] ? sha1($this->generate_token(20)) : '';
	...
	public function insert_custom_user_data($user_id = FALSE, $custom_data = FALSE)
	{
		...
		// Loop through custo data table(s) set via config file.
				...
				if (count($sql_update) > 0)
				{
					...
					{						
						...
						if( $foreign_key_exist ){
							$rst = $this->db->where($identifier_col, $identifier_id)->get($table_data['table']);
							if( !$rst->num_rows() ){
								$sql_update[ $identifier_col ] = $identifier_id;
								$this->db->insert( $table_data['table'], $sql_update );
							}
							$foreign_key_exist = false;
						}
					}
	...
	public function delete_privilege_user($sql_where)
  	{
		if (is_numeric($sql_where))
		{
			$sql_where = array($this->auth->tbl_col_user_privilege_users['user_id'] => $sql_where);
		}
	 ...
	 * @author Rob Hussey
	 */
	public function search_users($search_query = FALSE, $exact_match = FALSE, $sql_select = FALSE, $sql_where = FALSE, $sql_group_by = TRUE, $escape = TRUE)
	{
		...
		return $this->get_users($sql_select, $sql_where, $sql_group_by, $escape);
	}
	...
	{	
		...
		$query = $this->db->select($sql_select)
                        ->join('user_profiles','user_profiles.upro_uacc_fk = user_accounts.uacc_id')
                        ->where('user_profiles.upro_is_guest','0')
			->where($sql_where)
			->get($this->auth->tbl_user_account);
	...
	private function set_login_sessions($user, $logged_in_via_password = FALSE)
	{
		...
		$this->auth->session_data[$this->auth->session_name['user_privileges']] = $privileges;
		...
		// Set group privileges to session.
		$this->auth->session_data[$this->auth->session_name['group_privileges']] = $privileges;
		/*
		$sql_select = false;
		$sql_where = array($this->CI->auth->tbl_col_user_account['id'] => $user_id);
		*/
		$this->auth->session_data[$this->auth->session_name['custom_user_data']] = $this->flexi_auth->get_user_by_id_row_array();
		###+++++++++++++++++++++++++++++++++###
	...
	public function send_email($email_to = NULL, $email_title = NULL, $data = NULL, $template = NULL)
	{
		...
		if( defined( "MCC_EMAIL_FROM_TEMP_ACCOUNT" ) && MCC_EMAIL_FROM_TEMP_ACCOUNT ){
			$this->load->library('SEmail');

			$email_config = [
				'to' => $email_to,
				'subject' => 'MCC - Testing',
				'from' => 'testing-mcc@yopmail.com',
				'body' => $message
			];		
