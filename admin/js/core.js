...
function row_status(x) {
    ...
    } else if (x == 'completed' || x == 'paid' || x == 'sent' || x == 'received'||x == 'invoiced') {
...
function stock_status(x) {
    ...
    if (yellow == 1) {
        return '<div class="text-center"><span class="label label-warning">Yellow</span></div>';
    } else {
        if ((green == 1) && (red == 0)) {
            return '<div class="text-center"><span class="label label-success">Green</span></div>';
        } else if ((red == 1) && (green == 0)) {
            return '<div class="text-center"><span class="label label-danger">Red</span></div>';
        } else {
            return '<div class="text-center"><span class="label label-warning">Yellow</span></div>';
        }
    }
...
$(document).ready(function () {
    ...
    $('body').on('click', '.purchase_link td:not(:first-child :last-child)', function () {
     ...
     $('body').on('click', '.po_link td:not(:first-child :last-child)', function () {
        $('#myModal').modal({remote: site.base_url + 'sep/modal_po/' + $(this).parent('.po_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'purchases/view/' + $(this).parent('.purchase_link').attr('id');
    });
     $('body').on('click', '.sep_link td:not(:first-child :last-child)', function () {
        $('#myModal').modal({remote: site.base_url + 'sep/modal_sep/' + $(this).parent('.sep_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'purchases/view/' + $(this).parent('.purchase_eligible').attr('id');
    });
    ...
    $('body').on('click', '.proforma_link td:not(:first-child :last-child)', function () {
        $('#myModal').modal({remote: site.base_url + 'proforma/modal_view/' + $(this).parent('.proforma_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.proforma_link').attr('id');
    });
    $('body').on('click', '.sales_order td:not(:first-child :last-child)', function () {
        $('#myModal').modal({remote: site.base_url + 'order/modal_view/' + $(this).parent('.sales_order').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.proforma_link').attr('id');
    });
